...
{
	bool  b = false;	
	if (board->getTile(c)->is(DEADBOLT) || board->getTile(c)->is(LASER))
	{
		...
		if (board->getTile(c)->is(LASER) && (((this->getActionTokens() >= 3) && board->getTile(c)->isFlipped() == false) || ((this->getActionTokens() >= 2) && board->getTile(c)->isFlipped() == true))) b = true;
		{
			b = true;
		};
	}
