...
void GuardObserver::update()
{
	...
	if (pos != lastPos)
	{
		std::pair<int, int> target = std::pair<int, int>((int)(FLOOR_YPOS + TILE_POS_Y[pos.row][pos.col] + (TILE_SIZE - GUARD_SIZE) / 2),(int) (FLOOR_XPOS[pos.floor] + TILE_POS_X[pos.row][pos.col] + (TILE_SIZE - GUARD_SIZE) / 2));
		if (lastPos == NPOS)
		{
			...
			dices[lastSpeed]->setPosition((int)(FLOOR_YPOS + TILE_POS_Y[i][j] + (TILE_SIZE - DICE_SIZE) / 2),(int) (FLOOR_XPOS[pos.floor] + TILE_POS_X[i][j] + (TILE_SIZE - DICE_SIZE) / 2));
		}
		else
	...
	if (lastSpeed != guard->getSpeed() || lastTarget != guard->getTarget())
	{
		...
		int i = guard->getTarget().row;
		int j = guard->getTarget().col;
		dices[lastSpeed]->setPosition((int)(TILE_POS_Y[i][j] + (TILE_SIZE - DICE_SIZE) / 2),(int) (FLOOR_XPOS[pos.floor]+ TILE_POS_X[i][j] + (TILE_SIZE - DICE_SIZE) / 2));
