#include "../Header Files/FlipAnimation.h"

FlipAnimation::FlipAnimation(ObjectInterface* ob, double duration)
{
	this->duration = duration;
	initialFrames=framesLeft = ceil(duration * 30);
...
FlipAnimation::play(ObjectInterface* ob)
{
	...
	else
	{
		...
		{
			delete move;
			move =  new MoveAnimation(midPos, startPos, duration / 2.0);
			middle = true;
		}
		move->play(ob);
		ob->setScaleX(ob->getScales().second + rate);
